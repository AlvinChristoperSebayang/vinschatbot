...
def clean_text(text, training_data):
    tokens = word_tokenize(text.lower())
    stop_words = set(stopwords.words('english') + stopwords.words('indonesian'))
    filtered_tokens = [word for word in tokens if word.isalnum() and word not in stop_words]

    # Menggunakan kata-kata dalam data latih sebagai kata-kata yang benar
    valid_words = set(word for sentence, label in training_data for word in word_tokenize(sentence.lower()))

    cleaned_tokens = [find_similar_word(word, valid_words) for word in filtered_tokens]
...
def find_similar_word(word, word_list):
    return max(word_list, key=lambda x: fuzz.ratio(word, x))
...
for intent, questions_dict in questions.items():
    for lang, question_list in questions_dict.items():
        for question in question_list:
            features = extract_features(question,training_data)
            training_data.append((question, intent))
classifier = NaiveBayesClassifier.train(training_data)
# Fungsi untuk mengekstrak fitur dari teks
def extract_features(text, training_data):
    features = {}
    for word in clean_text(text, training_data):
        features[word] = True
        ...
        for topic, lang_questions in questions.items():
            for lang, question_list in lang_questions.items():
                similar_words = process.extract(word, clean_text(' '.join(question_list), training_data), limit=3)
